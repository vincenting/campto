...
var Topic = module.exports = function () {
};
...
Topic.prototype._rand = function () {
    ...
    this.subject = this._prepareSubject();
    return this;
};
...
Topic._numToChinese = function (num) {
    ...
    var units = num % 10, tens = Math.floor(num / 10);
    if (tens === 1) {
...
Topic.prototype._prepareSubject = function () {
    var currentLen = this._le[0] + this._rt[0];
    var optArr = chinese['operator'][this._operator];
    if (currentLen === 6) {
    ...
    var eqlLen = CAPTCHA_LEN - this._optLen - currentLen;
