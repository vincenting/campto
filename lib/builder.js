...
CaptchaBuilder.prototype.prepareForDraw = function () {
  ...
  this.randColorSet = options['colorSet'] || ['#000000', '#b50000', '#373000']

  // HELP NEEDED 为了避免每次生成验证码都进行 IO 文件读取，暂时不支持使用 glob 的方式传值
  this.randBackgroundSet = options['backgroundSet'] || [path.join(CAMPTO_ASSETS_PATH, 'images', 'bg.gif')]
  this.randFontFileSet = options['fontFileSet'] || Array(6).fill().map((_, idx) => {
    return path.join(CAMPTO_ASSETS_PATH, 'fonts', './0' + (idx + 1) + '.ttf')
  })
}
...
CaptchaBuilder.prototype.getBackgroundImagePath = function () {
  return _.sample(this.randBackgroundSet)
}

CaptchaBuilder.prototype.randFont = function () {
  return _.sample(this.randFontFileSet)
}
...
CaptchaBuilder.prototype.randColor = function () {
  return _.sample(this.randColorSet)
}
