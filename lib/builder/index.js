...
CaptchaBuilder.prototype.draw = function (callback) {
    var self = this;
    async.map(this._subject.split(' '), self.createTempWordFile.bind(self), function (err, wordsFiles) {
        if (err) return callback(err, null);
        // 将所有临时单个字母的图片拼凑成一张临时图片
        self.tempFile(function (err, tempPath) {
            if (err) return callback(err, null);
            var tempCaptcha = gm(wordsFiles.shift());
                ...
                .write(tempPath, function (err) {
                    ...
                    self.getBackgroundImage(function (err, backgroundPath) {
                        ...
                        callback(false, gm(backgroundPath)
                            .composite(tempPath).geometry('+10+0').setFormat('png'));
                    });
                });
        });
    });
