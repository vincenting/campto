...
CaptchaBuilder.prototype.draw = function (options) {
  ...
  return Promise.mapSeries(this.subject, this.createTempWordFile.bind(this, options))
    .then(wordsFiles => {
    ...
    return [captchaPath, this.createRandomLinesBackground(options)]
...
CaptchaBuilder.prototype.createRandomLinesBackground = function (options) {
  return this.tempFile().then(tempPath => {
    ...
    if (options.recognitionDifficulty === 'hard') {
      _.times(parseInt(this.captchaHeight / 10, 10), () => {
...
CaptchaBuilder.prototype.createTempWordFile = function (options, word) {
  ...
  return this.tempFile().then(tempPath => {
    ...
    if (recognitionDifficulty === 'hard') {
      wordLayer.affine([
        _.random(.9, 1.05), _.random(-.1, .05),
        _.random(-.1, .05), _.random(.9, 1.05),
        _.random(-.2, .1), _.random(-.2, .1)
      ]).transform()
    }
    return wordLayer.writeAsync(tempPath)
      .return(tempPath)
  })
}
